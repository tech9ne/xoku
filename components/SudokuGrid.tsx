...
const H = ["FFC059","F7DE8F","B1A5F3","DCD4FC","B22222","86F280","1E4BE8","4B0082","FFFF00"];
...
export default function SudokuGrid({ game, sel, step, showCands, digitFilter, filterMode, manualColors, brush, onPaintCand, onSelect, onCandClick, colorMode, onPaintCell, wingFilter }: Props & { colorMode: "default" | "cands" | "cells" | "links";
  ...
  return (
    <div className="relative w-full aspect-square select-none touch-manipulation">
          ...
          return (
            <div key={i}
              ...
              {value !== 0 ? (
                ...
                <div className={"grid grid-cols-3 grid-rows-3 w-full h-full text-[#646464] text-[9px] sm:text-[11px] leading-none " + (wingFilter > 0 && (game.cands[i].toString(2).match(/1/g)?.length ?? 0) === wingFilter ? FILTER_BG : "")}>
                  {ALL_DIGITS.map(d => {
